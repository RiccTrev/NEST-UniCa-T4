...
#def objective_rule(model):
#    return sum(-model.C_sell * model.P_GRID_sell[t] + model.C_buy * model.P_GRID_buy[t] for t in model.T)
#
#model.obj = Objective(rule=objective_rule, sense=minimize)

# New Constraints for Power Sold and Bought
#def power_sell_rule(model, t):
#    return model.P_GRID_sell[t] >= model.P_GRID[t]
#
#model.power_sell_constraint = Constraint(model.T, rule=power_sell_rule)
#
#def power_buy_rule(model, t):
#    return model.P_GRID_buy[t] >= -model.P_GRID[t]
#
#model.power_buy_constraint = Constraint(model.T, rule=power_buy_rule)
